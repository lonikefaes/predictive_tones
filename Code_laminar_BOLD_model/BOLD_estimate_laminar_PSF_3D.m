function est_kernel = BOLD_estimate_laminar_PSF_3D(N,K,MAT_ind,depth_map_HR,depth_map,ind_vox_sel)
...
disp('Estimating voxel blurring for given number of BOLD depths...')
...
IF = reshape(IF,[size(IF,1)*size(IF,2)*size(IF,3),size(IF,4)]);
