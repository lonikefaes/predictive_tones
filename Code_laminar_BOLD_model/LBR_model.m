function [LBR,Y,LBRpial,LBR_v,LBR_d] = LBR_model(P,cbf,cmro2)
...
for t = 1:P.T/dt
    ...
    LBR_v(t,:) = (1./(1 - V0vq + ep_v.*V0vq)).*((1-V0vq).*(k1v.*V0vq.*(1-q_v))+k2v.*V0vq.*(1-q_v./v_v)+k3v.*V0vq.*(1-v_v)).*100;
    
    LBR_d(t,:)   = (1./(1 - V0dq + ep_d.*V0dq)).*((1-V0dq).*(k1d.*V0dq.*(1-q_d))+k2d.*V0dq.*(1-q_d./v_d)+k3d.*V0dq.*(1-v_d)).*100;
