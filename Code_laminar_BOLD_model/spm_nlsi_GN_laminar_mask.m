function [Ep,Cp,Eh,F,L,dFdp_msk,dFdpp,Drift] = spm_nlsi_GN(M,U,Y)
...
ny_masked = length(M.Mask(:));
ns_masked = length(M.Mask(:,1));
...
nq    = ny_masked/length(Q{1});            % for compact Kronecker form of M-step
...
% catch
    dfdu_msk = sparse(ny_masked,0);
%end
if isempty(dfdu_msk), dfdu_msk = sparse(ny_masked,0); end
...
%--------------------------------------------------------------------------
V     = spm_svd(pC,0);
nu    = size(dfdu_msk,2);                 % number of parameters (confounds)
...
%--------------------------------------------------------------------------
Eu    = spm_pinv(dfdu_msk)*spm_vec(y(M.Mask));
...
for k = 1:M.Nmax
    ...
    try
        ...
        f_msk = f(M.Mask); dfdp_msk=[];
        for ii = 1:np
            dfdp_msk{1,ii} = dfdp{1,ii}(M.Mask);
        end
        dfdp_msk     = reshape(spm_vec(dfdp_msk),ny_masked,np);
        
        % check for stability
        %------------------------------------------------------------------
        normdfdp_msk = norm(dfdp_msk,'inf');
        revert   = isnan(normdfdp_msk) || normdfdp_msk > exp(32);
    ...
    e     =  spm_vec(y(M.Mask)) - spm_vec(f(M.Mask)) - dfdu_msk*p(iu);
    ...
    %----------------------------------------------------------------------
    L(1) = spm_logdet(iS)*nq/2  - real(e'*iS*e)/2 - ny_masked*log(8*atan(1))/2;            ...
    ...
    if exist('Fsi', 'var')
        ...
        if isreal(spm_vec(y))
            ...
            f_update = f; f_update(M.Mask) = f(M.Mask) +e;
            plot(x,y,':'), hold off
            xlabel(xLab)
