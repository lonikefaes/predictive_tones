function [Ep,Cp,Eh,F,L,dFdp,dFdpp,Drift] = spm_nlsi_GN(M,U,Y)
...
nr1   = size(y{1},2);                      % number response components
nr2   = size(y{2},2);                    % number response components
M.ns1 = ns1;                          % number of samples M.ns
M.ns2 = ns2;  
% initial states
%--------------------------------------------------------------------------
try
...
% confounds (if specified) !!!!! has to be changed
%--------------------------------------------------------------------------
try
   % nb1   = size(Y.X0{1},1);            % number of bins
    %nx1   = nr1;                   % number of blocks
    dfdu = blkdiag(kron(speye(nr1,nr1),Y.X0{1}),kron(speye(nr2,nr2),Y.X0{2}));
catch
    dfdu = sparse(ny,0);
end
...
p     = [V'*(spm_vec(M.P) - spm_vec(M.pE)); Eu*0];
...
for k = 1:M.Nmax
    ...
    figure(100)
    [yb Xb] = spm_int_IT2(Ep,M,U);
   subplot(311), plot(Xb(:,1:8));
   subplot(312), plot(exp(Xb(:,17:24)));
   subplot(313), plot(exp(Xb(:,33:40)));
    drawnow;
    % Graphics
    %======================================================================
    if exist('Fsi', 'var')
