...
for k = 1:M.Nmax
    ...
    try
        ...
        % gradients (using forward difference)
        %------------------------------------------------------------------
      %  pc = p(ip,ones(1,np)) + dI;
        f  = spm_int_IT(Ep,M,U);
        dfdp  = cell(1,np);
        for i = 1:np
            Epi     = spm_unvec(spm_vec(Ep) + V(:,i)*dx,pE);
            fi      = spm_int_IT(Epi,M,U);
            dfdp{i} = (fi - f)/dx;
        end    
   %    [dfdpi fi] = spm_diff(IS,Ep,M,U,1,{V});
    ...
    if revert && k > 1
        for i = 1:4
            ...
            try
                ...
                dfdp  = cell(1,np);
                for j = 1:np
                   Epi     = spm_unvec(spm_vec(Ep) + V(:,j)*dx,pE);
                   fi      = spm_int_IT(Epi,M,U);
                   dfdp{j} = (fi - f)/dx;
                end
                dfdp     = reshape(spm_vec(dfdp),ny,np);
